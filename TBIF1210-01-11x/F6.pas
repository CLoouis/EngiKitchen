	...
	procedure olahBahan(var tSimulasi : tabSimulasi; var tInventoriBahanMentah : tabInventoriBahanMentah; var tBahanOlahan : tabBahanOlahan;
		...
		{kamus lokal}
			var
					...
					valid : boolean; //variable penanda boolean
								...
								repeat //looping untuk inventori bahan mentah
										...
										repeat // looping bahan yg diperlukan untuk diolah
												if (tInventoriBahanMentah.tab[i].NamaBahanMentah = tBahanOlahan.tab[n].bahan[j]) then
													begin
															if 	tInventoriBahanMentah.tab[i].Jumlah >= 1 then
																begin 
												...
												j := j + 1;
										until
											(j = tBahanOlahan.tab[n].JumlahBahan + 1); 
											
										i:= i + 1;
								until
									(i = tInventoriBahanMentah.neff + 1);
									
								if (banyakTerolah < tBahanOlahan.tab[n].JumlahBahan) then //jika barang tidak cukup
									begin
											writeln('Pembelian gagal, silahkan lihat kembali bahan yang diperlukan dan inventori anda.');
									end
