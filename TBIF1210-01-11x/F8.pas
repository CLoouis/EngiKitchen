	...
	begin
		...
		begin
			...
			begin
				found := False;
				k := 1;
				while ( k <= tInventoriBahanMentah.neff ) and (not found) do // k adalah loop untuk mencari bahan dalam inventori
				begin
					if ( tResep.tab[i].bahan[j] = tInventoriBahanMentah.tab[k].NamaBahanMentah) then
					begin
						if (tInventoriBahanMentah.tab[k].Jumlah	>= 1 ) then
						begin
							found := True;
							c := c + 1;
						end else 
				...
				k := 1;
				while ( k <= tInventoriBahanOlahan.neff ) and ( not found) do
				begin
					if ( tResep.tab[i].bahan[j] = tInventoriBahanOlahan.tab[k].NamaBahanOlahan) then
					begin
						if (tInventoriBahanOlahan.tab[k].Jumlah >= 1) then
						begin
							found := True;
							c := c + 1;
						end else
